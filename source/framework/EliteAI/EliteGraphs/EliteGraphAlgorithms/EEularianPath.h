...
namespace Elite
{
	...
	inline std::vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		{
			for (int i =0 ; i < graphCopy->GetNrOfNodes() ; i++)
			{
				if (graphCopy->GetNodeConnections(graphCopy->GetNode(i)).size() & 1)
				{
					++nrOfUnevenNodes;
					unevenNodeIdx = i;
				}
			...
			if (nrOfUnevenNodes != 0 && nrOfUnevenNodes != 2)
			{
				return path;
			}
			else if (nrOfUnevenNodes == 2)
			{
			...
			int currentNodeIdx{ unevenNodeIdx };

			while (!(graphCopy->GetNodeConnections(graphCopy->GetNode(currentNodeIdx)).size() == 0 && nodeStack.size() == 0))
			{
				if (graphCopy->GetNodeConnections(graphCopy->GetNode(currentNodeIdx)).size() != 0)
				{

					int prevNodeIdx = currentNodeIdx;
					//I
					nodeStack.push(currentNodeIdx);

					//II + III
					currentNodeIdx = graphCopy->GetNodeConnections(currentNodeIdx).front()->GetTo();


					//IV
					graphCopy->RemoveConnection(prevNodeIdx,currentNodeIdx);
