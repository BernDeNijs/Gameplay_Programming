...
void App_GraphTheory::Update(float deltaTime)
{
	...
	auto sortedPath = eulerFinder.FindPath(eulerianity);

	switch (eulerianity)
	{
	case Elite::Eulerianity::notEulerian:
		cout << "not eularian" << endl;

		break;
	case Elite::Eulerianity::semiEulerian:
		cout << "semi eularian" << endl;
		
			for (int pathIdx = 0; pathIdx < sortedPath.size(); pathIdx++)
			{
				Elite::Color gradient{ (pathIdx * (1.f / sortedPath.size())),0,0 };
				m_pGraph2D->GetNode(sortedPath[pathIdx]->GetIndex())->SetColor(gradient);
			}
